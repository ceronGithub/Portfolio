...
const FORWARD_VIDEO    = "https://drive.google.com/uc?export=download&id=1uiulY9lRlVfEwmRpfmllSV4ZrBeQrP3T";
const REVERSE_VIDEO    = "https://drive.google.com/uc?export=download&id=1wo5JBF6qz062hZ4ixKpOrLHzBR7qypQB";
      ...
      >
        ...
        <div className="modelIntroTickerWrap">
          <p className="modelIntroTicker">
            3D CHARACTER &amp; WEAPON MODELING
          </p>
        </div>
        <div className="modelIntroSlogan">
          {TAGLINES.map((line, i) => (
            <p
              key={i}
              ref={(el: HTMLParagraphElement | null) => { lineRefs.current[i] = el; }}
              className="modelIntroLine"
            >
              {line}
            </p>
          ))}
        </div>
        <div className="modelIntroBottomGrad" />
      </div>
    </>
  );
}