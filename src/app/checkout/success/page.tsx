...
// Calls /api/checkout/fulfill on mount to mark orders PAID + unlock downloads.
// Works on localhost (no webhook needed) and is idempotent on production.
"use client";
...
function ToastBanner() {
  ...
  useEffect(() => {
    const s = setTimeout(() => setVisible(true),  300);
    const h = setTimeout(() => setVisible(false), 4500);
...
export default function CheckoutSuccessPage() {
  ...
  useEffect(() => {
    if (fulfilled.current) return;
    fulfilled.current = true;

    // Parse order IDs from ?orders= param
    const raw      = searchParams.get("orders") ?? "";
    const orderIds = raw.split(",").map(s => s.trim()).filter(Boolean);
    if (orderIds.length === 0) return;

    // Call fulfill API — marks orders PAID + creates Ownership
    fetch("/api/checkout/fulfill", {
      method:  "POST",
      headers: { "Content-Type": "application/json" },
      body:    JSON.stringify({ orderIds }),
    }).catch(err => console.error("[CheckoutSuccess] fulfill error:", err));
  }, [searchParams]);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ color:"rgba(255,255,255,0.5)", fontSize:"0.9rem", lineHeight:1.6, marginBottom:"2rem" }}>
          Your payment was received. Your downloads are being unlocked — check your library now.
        </p>
